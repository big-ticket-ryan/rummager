...
### Recommended Realtime Database rules — full ruleset (replace ALL existing rules)

In the Firebase Console → Realtime Database → Rules, paste this entire block. It keeps the v2.1 groups + admins blocks intact, tightens `publicStops` to the Ozaukee bbox, validates the new Fredonia fields (hours, priceSlash, name, sellerName, pinVerified), and adds the `publicStats` block needed by the usage-stats dashboard.
